...
|---|---|---|
| No url field in request body | 400 | `{ "error": "URL is required" }` |
| URL is invalid or unreachable | 400 | `{ "error": "Failed to fetch the URL" }` |
| Request exceeds 10 second timeout | 408 | `{ "error": "Request timed out after 10 seconds" }` |
| Response is not an HTML page | 422 | `{ "error": "URL did not return an HTML page" }` |
